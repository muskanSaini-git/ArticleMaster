...
// Generate static sample data for approved parcels
const generateApprovedData = () => {
  const data = [];
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    data.push({
      ...
      Images: [
        ...
        `https://picsum.photos/100/100?random=${i + 101}`,
        `https://picsum.photos/100/100?random=${i + 102}`
      ],
...
const Approvals = ({ onSidebarToggle }) => {
  ...
  const loadStaticData = () => {
    ...
    setTimeout(() => {
      const staticData = generateApprovedData();
  ...
  const handleEdit = (approval) => {
    ...
    showToast(`✏️ Editing article ${approval.ArticleId}`, 'info');
  };

  const handleEditApprove = async () => {
    if (!editApproval) return;
    
    setActionLoading(`edit-approve-${editApproval.ArticleId}`);
    try {
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      setArticleRows(prevRows => 
        prevRows.map(row => 
          row.ArticleId === editApproval.ArticleId 
            ? { ...row, STATUS: 'Approved' }
            : row
        )
      );
      
      showToast(`Article ${editApproval.ArticleId} approved successfully!`, 'success');
      setEditApproval(null);
      setRejectionReason('');
    } catch (error) {
      showToast(`Failed to approve article ${editApproval.ArticleId}`, 'error');
    } finally {
  ...
  const handleEditReject = async () => {
    ...
    setActionLoading(`edit-reject-${editApproval.ArticleId}`);
    try {
      await new Promise(resolve => setTimeout(resolve, 1000));
      ...
      showToast(`Article ${editApproval.ArticleId} rejected successfully!`, 'error');
      setEditApproval(null);
      setRejectionReason('');
    } catch (error) {
      showToast(`Failed to reject article ${editApproval.ArticleId}`, 'error');
    } finally {
      setActionLoading(null);
    }
  };
  ...
  return (
    <div className="article-parcel-container">
      ...
      <TopBar onSidebarToggle={onSidebarToggle} currentPage="approvals" />
      ...
      {/* Edit Modal */}
      <EditModal
        isOpen={editApproval !== null}
        onClose={() => setEditApproval(null)}
        editItem={editApproval}
